...
class training(db.Model):
    ...
    preferred_package=db.Column(db.String(50))
    start_date=db.Column(db.String(5))
...
def addbeeproduct():
    if request.method=="POST":
        ...
        products=Addbeeproducts(username=username,email=email,productname=productname,productdesc=productdesc,price=price)
...
@login_required
def training():
    training=db.engine.execute("SELECT * FROM `training`")
    if request.method=="POST":
        Name=request.form.get('Name')
        preferred_package=request.form.get('Preferred Package')
        start_date=request.form.get('Start date')
        Cost=request.form.get('Cost')     
        query=db.engine.execute(f"INSERT INTO `training` (`Name`,`preferred_package`,`start_date`,`Cost`) VALUES ('{Name}','{preferred_package}','{start_date}','{Cost}')")
        flash("Your Record Has Been Saved","success")
